...
plan: 04
subsystem: api
tags: [services, mint-burn, issuance, idempotency, worker]
requires:
  - phase: 07-01
    provides: shared service contracts and persistence baseline
provides:
...
affects: [phase-07-backend-services, mint-burn, indexer, webhook]
tech-stack:
  added: []
  patterns: [issuer-only-authz, durable-idempotency, async-job-lifecycle, normalized-internal-events]
key-files:
  created:
...
patterns-established:
  - "Mutating issuance admission must pass tenant scope, issuer-only auth, and idempotency guard before queueing."
  - "Idempotency conflicts are deterministic by key+payload fingerprint mismatch."
