...
plan: 05
subsystem: api
tags: [services, webhook, retries, dlq, signatures]
requires:
  - phase: 07-02
    provides: finalized indexer event envelopes and tenant-scoped projections
  - phase: 07-04
    provides: issuer-authenticated request context and durable idempotency baseline
provides:
...
affects: [phase-07-backend-services, webhook, indexer, integrations]
tech-stack:
  added: []
  patterns: [per-entity-ordering, bounded-exponential-retry, dead-letter-contract, dual-key-rotation]
key-files:
  created:
...
patterns-established:
  - "Delivery attempts always persist deterministic state transitions: queued/retry_scheduled -> delivering -> succeeded|retry_scheduled|dead_lettered."
  - "Retention purge is explicit and deterministic at 90d (deliveries) and 180d (dead letters)."
