...
BLACKLIST_REASON="${BLACKLIST_REASON:-phase-08-proof}"
TOKEN_NAME="${TOKEN_NAME:-Phase08 SSS2}"
TOKEN_SYMBOL="${TOKEN_SYMBOL:-P8S2}"
TOKEN_URI="${TOKEN_URI:-https://example.com/sss2.json}"
...
if [ -z "$RUN_ID" ]; then
  echo "error: RUN_ID is required (example: RUN_ID=rerun-001)." >&2
  exit 2
fi
