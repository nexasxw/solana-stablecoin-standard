...
BLACKLISTER_SIGNER="${BLACKLISTER_SIGNER:-$AUTHORITY_SIGNER}"
SEIZER_SIGNER="${SEIZER_SIGNER:-$AUTHORITY_SIGNER}"
TARGET_TOKEN_ACCOUNT="${TARGET_TOKEN_ACCOUNT:-}"
TARGET_OWNER="${TARGET_OWNER:-}"
TREASURY_TOKEN_ACCOUNT="${TREASURY_TOKEN_ACCOUNT:-}"
BLACKLIST_REASON="${BLACKLIST_REASON:-phase-10-proof}"
TOKEN_NAME="${TOKEN_NAME:-Phase10 SSS2}"
TOKEN_SYMBOL="${TOKEN_SYMBOL:-P10S2}"
TOKEN_URI="${TOKEN_URI:-https://example.com/phase10-sss2.json}"
TOKEN_DECIMALS="${TOKEN_DECIMALS:-6}"
MINT_QUOTA="${MINT_QUOTA:-1000000000}"
MINT_AMOUNT="${MINT_AMOUNT:-1000}"
NEGATIVE_PATH_REASON="${NEGATIVE_PATH_REASON:-seize-without-blacklist}"
...
if [ -z "$AUTHORITY_SIGNER" ] || [ -z "$TARGET_TOKEN_ACCOUNT" ] || [ -z "$TARGET_OWNER" ] || [ -z "$TREASURY_TOKEN_ACCOUNT" ]; then
  echo "error: AUTHORITY_SIGNER, TARGET_TOKEN_ACCOUNT, TARGET_OWNER, TREASURY_TOKEN_ACCOUNT are required." >&2
  exit 2
fi
if [ ! -f "$AUTHORITY_SIGNER" ] || [ ! -f "$MINTER_SIGNER" ] || [ ! -f "$PAUSER_SIGNER" ] || [ ! -f "$BLACKLISTER_SIGNER" ] || [ ! -f "$SEIZER_SIGNER" ]; then
...
ARTIFACT_DIR="$ARTIFACT_ROOT/sss2-proof/$RUN_ID"
...
INIT_JSON="$(run_json init \
  ...
  init --preset sss-2 --name "$TOKEN_NAME" --symbol "$TOKEN_SYMBOL" --uri "$TOKEN_URI" --decimals "$TOKEN_DECIMALS")"
record_signature "init" "$INIT_JSON"
MINT_ADDRESS="$(extract_data_field "$INIT_JSON" "data.mint")"

run_json state-after-init \
  "$CLI" --json --rpc-url "$RPC_URL" --mint "$MINT_ADDRESS" --variant SSS_2 \
...
MINT_JSON="$(run_json mint \
  "$CLI" --json --rpc-url "$RPC_URL" --mint "$MINT_ADDRESS" --variant SSS_2 \
  --minter-signer "$MINTER_SIGNER" -y mint "$TARGET_TOKEN_ACCOUNT" "$MINT_AMOUNT")"
record_signature "mint" "$MINT_JSON"

FREEZE_JSON="$(run_json freeze \
  "$CLI" --json --rpc-url "$RPC_URL" --mint "$MINT_ADDRESS" --variant SSS_2 \
  --pauser-signer "$PAUSER_SIGNER" -y freeze "$TARGET_TOKEN_ACCOUNT")"
record_signature "freeze" "$FREEZE_JSON"

NEGATIVE_PATH_JSON="$(run_expected_failure negative-path-seize-without-blacklist \
  "$CLI" --json --rpc-url "$RPC_URL" --mint "$MINT_ADDRESS" --variant SSS_2 \
  --seizer-signer "$SEIZER_SIGNER" -y seize "$TARGET_TOKEN_ACCOUNT" "$TARGET_OWNER" --to "$TREASURY_TOKEN_ACCOUNT")"
cp "$NEGATIVE_PATH_JSON" "$ARTIFACT_DIR/state/negative-path-seize-without-blacklist.json"

BLACKLIST_ADD_JSON="$(run_json blacklist-add \
  "$CLI" --json --rpc-url "$RPC_URL" --mint "$MINT_ADDRESS" --variant SSS_2 \
  --blacklister-signer "$BLACKLISTER_SIGNER" -y blacklist add "$TARGET_OWNER" --reason "$BLACKLIST_REASON")"
record_signature "blacklist_add" "$BLACKLIST_ADD_JSON"

run_json blacklist-check \
  "$CLI" --json --rpc-url "$RPC_URL" --mint "$MINT_ADDRESS" --variant SSS_2 \
  blacklist check "$TARGET_OWNER" >"$ARTIFACT_DIR/state/blacklist-check.json"

SEIZE_JSON="$(run_json seize \
  "$CLI" --json --rpc-url "$RPC_URL" --mint "$MINT_ADDRESS" --variant SSS_2 \
  --seizer-signer "$SEIZER_SIGNER" -y seize "$TARGET_TOKEN_ACCOUNT" "$TARGET_OWNER" --to "$TREASURY_TOKEN_ACCOUNT")"
record_signature "seize" "$SEIZE_JSON"

BLACKLIST_REMOVE_JSON="$(run_json blacklist-remove \
  "$CLI" --json --rpc-url "$RPC_URL" --mint "$MINT_ADDRESS" --variant SSS_2 \
  --blacklister-signer "$BLACKLISTER_SIGNER" -y blacklist remove "$TARGET_OWNER")"
record_signature "blacklist_remove" "$BLACKLIST_REMOVE_JSON"
...
ENDED_AT="$(date -u +"%Y-%m-%dT%H:%M:%SZ")"
{
  ...
  echo "TARGET_TOKEN_ACCOUNT=$TARGET_TOKEN_ACCOUNT"
  echo "TARGET_OWNER=$TARGET_OWNER"
  echo "TREASURY_TOKEN_ACCOUNT=$TREASURY_TOKEN_ACCOUNT"
  echo "NEGATIVE_PATH_LABEL=negative-path-seize-without-blacklist"
  echo "NEGATIVE_PATH_REASON=$NEGATIVE_PATH_REASON"
  echo "NEGATIVE_PATH_FILE=state/negative-path-seize-without-blacklist.json"
  echo "STARTED_AT=$STARTED_AT"
  echo "ENDED_AT=$ENDED_AT"
} >"$METADATA_FILE"

cat >"$SUMMARY_FILE" <<EOF
# Phase 10 SSS-2 Proof Summary
...
- negative-path:
  - label: negative-path-seize-without-blacklist
  - reason: $NEGATIVE_PATH_REASON
  - expected failure artifact: state/negative-path-seize-without-blacklist.json
- snapshots:
...
echo "SSS-2 phase-10 proof run complete."
